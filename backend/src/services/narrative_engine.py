"""Per-User Narrative Engine — rule-based + optional LLM enhancement."""
...
class AINarrativeGenerator:
    """Generate per-user narratives using rule-based + knowledge base."""
    ...
    def _gen_career(self, chart: dict, lang: str) -> list:
        ...
        if jupiter and jupiter.get("house") == 10:
            if lang == "th":
                paras.append("พฤหัสอยู่บ้านที่ 10 — การงานที่โชคดี ได้รับการยอมรับในสังคม")
            else:
...
def generate_transit_natal_narrative(chart: dict, transit_chart: dict | None = None, lang: str = "th") -> dict:
    return ai_generator.generate_full_reading(chart, user_id="", lang=lang)


def generate_synastry_narrative(chart1: dict, chart2: dict, lang: str = "th") -> dict:
    return ai_generator.generate_full_reading(chart1, user_id="", lang=lang)
