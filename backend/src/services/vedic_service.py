...
def compute_vedic_chart(
    ...
    **kwargs,
    ...
    """Compute Vedic chart from structured inputs. Compatible with compute_vedic."""
    from datetime import date as date_type, time as time_type

    if isinstance(birth_date, date_type) and isinstance(birth_time, time_type):
        birth_datetime_local = f"{birth_date.isoformat()}T{birth_time.isoformat()}"
    elif isinstance(birth_date, str):
        birth_datetime_local = birth_date
    else:
        birth_datetime_local = f"{birth_date}T{birth_time}"
...
def compute_dasha_periods(
    ...
    **kwargs,
    ...
    """Compute Vimshottari dasha periods. Delegates to fusion_engine.vimshottari_now."""
    from datetime import date as date_type, time as time_type, datetime, timedelta
    from src.services.fusion_engine import vimshottari_now, _sidereal as sidereal_fn
    ...
    chart = compute_chart("vedic", utc_dt.date(), utc_dt.time(),
                          tz_offset_hours=0, lat=lat, lon=lon)
    planets = {p["body"]: p["absolute_deg"] for p in chart["bodies"]}
    moon_sid = sidereal_fn(planets["Moon"], utc_dt)

    birth_year_frac = utc_dt.year + (utc_dt.timetuple().tm_yday / 365.25)
    now_year_frac = birth_year_frac  # at birth

    dasha = vimshottari_now(moon_sid, birth_year_frac, now_year_frac)
    return {"dasha": dasha}
