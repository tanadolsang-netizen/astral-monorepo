...
def sky_view_ep(r: SkyIn) -> dict:
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Sky computation failed: {exc}") from exc
...
def life_tracking_ep(r: LifeIn) -> dict:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Life tracking computation failed: {exc}") from exc
