...
def test_weights_boost_the_lacking_element():
    ...
    weights = _weights_for(balance)

    lacking = balance.get("lacking", "น้ำ")
    dom = balance.get("dominant", "ไฟ")
    suffix_map = {"ไฟ": "Wands", "ดิน": "Pentacles", "ลม": "Swords", "น้ำ": "Cups"}
    lacking_w = [w for c, w in zip(FULL_DECK, weights) if c["name"].endswith(f"of {suffix_map.get(lacking, 'Cups')}")]
    dom_w = [w for c, w in zip(FULL_DECK, weights) if c["name"].endswith(f"of {suffix_map.get(dom, 'Wands')}")]
    if lacking_w and dom_w:
        assert min(lacking_w) >= max(dom_w)
    else:
        pass
...
def test_weighted_draw_has_no_duplicates():
    ...
    assert result["tilted_toward"] == "ลม"
...
def test_api_draw_with_birth_data_reports_tilt():
    ...
    body = res.json()
    assert body["tilted_toward"] == "ลม"
    assert body["elements"]["dominant"] == "ไฟ"
